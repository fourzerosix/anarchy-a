#define _GNU_SOURCE
/*
 * anarchy-a  --  Draw the Circle-A anarchy symbol in your terminal.
 *
 ...
 *             row = oy + sq_y * scale
 *
 * Build: gcc -O2 -o anarchy-a src/anarchy-a.c -lm
 */
...
static void usage(const char *prog){
    printf(
      ...
      "  -h, --help        Help\n\n"
      "Examples:\n"
      "  anarchy-a           Full animation\n"
      "  anarchy-a -p        Punk mode -- graffiti A with blood drip\n"
      "  anarchy-a -b        Builds then explodes (Bakunin)\n"
      "  anarchy-a -k        Two symbols slide in and meet (Kropotkin)\n"
      "  anarchy-a -n        Fire finale with No gods, no masters\n"
      "  anarchy-a -c -b     Crass + Bakunin: chaotic red destruction\n",
      prog);
}
...
static void draw_one_at(int scale, int ox, int oy, int delay, int plain,
    ...
    /* punk: crossbar blasts past the circle on both sides */
    if(g_punk){ blx=0; brx=18; }
...
static void draw_anarchy(const Opts *o) {
    ...
    if(!g_quit&&!o->bakunin&&!o->punk&&!o->nogods){
        const char *tag=o->crass?"~ anarchy-a ~  [ Do what thou wilt ]"
                                :"~ anarchy-a ~";
